...
export default function Gallery() {
  ...
  const y3 = useTransform(scrollYProgress, [0, 1], [0, height * .65])
...
const Column = ({ images, y }: { images: string[]; y: MotionValue }) => {
  return (
    <motion.div className='relative w-1/4 h-full min-w-3xs flex flex-col gap-[2vw] whitespace-nowrap' style={{ y }}>
      {images.map((src, i) => {
        return (
          <div key={i} className='h-1/3 w-full relative overflow-hidden rounded-[.5vw]'>
